...
function bench_client {
  ...
  echo "[+] Benchmarking PoC success rate against $CLIENT_IMPL_NAME client using PoC proxy at 18.136.101.97:$POC_PORT"
  case $CLIENT_IMPL in
    1|2)
      ...
      do
        docker run \
          ...
          $CLIENT_IMAGE -o Ciphers=aes128-cbc -o MACs=hmac-sha2-256-etm@openssh.com -p $POC_PORT victim@18.136.101.97 > /dev/null 2>&1
    ...
    3)
      ...
      do
        docker run \
          --network host \
          --name "$CLIENT_CONTAINER_NAME" \
          $CLIENT_IMAGE -P $POC_PORT -batch victim@18.136.101.97 > /dev/null 2>&1
